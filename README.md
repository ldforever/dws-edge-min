...
前端页签按"现场任务"分：监控类（实时监控 / 设备信息 / 历史查询 / 统计 / 诊断）+ 配置类
（**相机** / **输出对接** / **规则** / **系统与安全**）。原先把 10 个配置板块塞在一页里的"配置页"已经拆开；
"一键应用配置"提到页头下方的**全局应用条**（`frontend/src/applybar.ts`）：改了触发模式或相机清单就变黄，
点一下写配置 → 启动 SDK 回读校验 → 失败自动回滚，过程中显示进度，结果就在条上。
