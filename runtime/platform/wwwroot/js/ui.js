...
const TOAST_TTL = { ok: 3000, warn: 6000, err: 0 };
 ...
 */
export function inferKind(message) {
    const text = message.trim();
    if (OK_PATTERNS.some((pattern) => pattern.test(text)))
        return "ok";
    if (ERR_TAIL.test(text) || ERR_WORDS.test(text))
        return "err";
    return "warn";
}
let layer = null;
...
function armTimer(item) {
    if (item.timer !== null || item.remaining <= 0)
        return;
 ...
 */
export function toast(message, kind = "warn") {
    ...
    const item = { root, timer: null, remaining: TOAST_TTL[kind], startedAt: 0 };
...
export function confirmBox(message, options = {}) {
    ...
    return new Promise((resolve) => {
        ...
        function onKeyDown(event) {
            if (dialogStack[dialogStack.length - 1] !== onCancel)
                return;
            if (event.key === "Escape") {
        ...
        mask.addEventListener("click", (event) => {
            if (event.target === mask)
                finish(false); // 只有点在遮罩上才算取消，点对话框里不算
        });
