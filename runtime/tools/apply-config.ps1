...
# 兄弟脚本的目录：优先"脚本自己所在的目录"（发布时 tools 可能被整目录拷进 runtime\tools，
# 这时脚本和 set-trigger-mode.ps1 / make-camera-cfg.ps1 是邻居），
# 找不到再退回 <runtime 的上一级>\tools（开发时的仓库布局）。
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$toolsDir = $scriptDir
if (!(Test-Path (Join-Path $toolsDir 'set-trigger-mode.ps1'))) {
