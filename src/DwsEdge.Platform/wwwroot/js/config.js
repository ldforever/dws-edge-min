...
import { $, badge, cell, clear, dash, el, notify, positionLabel } from "./dom.js?v=802163b8";
import { confirmBox, toast } from "./ui.js?v=802163b8";
import { applyBarError, applyBarFinish, applyBarStart, initApplyBar, refreshApplyBar, setApplyBaseline, triggerModeToUi } from "./applybar.js?v=802163b8";
const TRIGGER_LABEL = {
    hard: "硬触发（光电）",
    soft: "软触发",
    free: "自由拉流（狂扫）"
...
function renderApplyResult(result, header = "") {
    ...
    badge(badgeNode, "退出码 " + result.exitCode + " · " + result.conclusion + (reason ? "：" + reason : ""), exitKind(result.exitCode));
...
async function showTemplateDiff(name) {
...
async function applyTemplate() {
    ...
    const body = {
        name: templatePicked,
        applyCameras: $("tplApplyCameras").checked,
        applyTrigger: $("tplApplyTrigger").checked,
        applyStorage: $("tplApplyStorage").checked,
        skipVerify: $("tplApplySkipVerify").checked,
        stopHost: $("cfgStopHost").checked,
        restartHost: $("cfgRestartHost").checked
    };
