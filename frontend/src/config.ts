...
async function applyConfig(): Promise<void> {
  ...
  let skipVerify = $<HTMLInputElement>("cfgSkipVerify").checked;
 ...
 */
async function precheckBeforeApply(cameras: string[] | null): Promise<"ok" | "skip" | "cancel"> {
  ...
  const skip = await confirmBox(
    "设备当前不可用，现在点「应用」会先停掉采集宿主，而且 SDK 校验必定失败并回滚：\n\n" +
      reasons.join("\n") +
...
function markSkipVerify(): void {
  const box = $<HTMLInputElement>("cfgSkipVerify");
  box.checked = true;
