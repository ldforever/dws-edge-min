...
import type { BarcodeRule, BarcodeRuleSet, RulesResponse } from "./types.js";

const ACTION_KEEP = "keep";
const ACTION_DROP = "drop";

let current: RulesResponse | null = null;
/** 表格里正在编辑的规则（保存/测试都用它） */
let editing: BarcodeRule[] = [];
...
function renderMeta(): void {
  if (!current) return;
  $<HTMLSelectElement>("ruleDefaultAction").value = current.defaultAction;
  $<HTMLInputElement>("ruleIgnoreCase").checked = current.ignoreCase;
...
function textInput(value: string | number | null | undefined, size: number, placeholder: string): HTMLInputElement {
...
function renderRuleRows(): void {
  const body = $<HTMLTableSectionElement>("ruleRows");
  clear(body);
  ...
  editing.forEach((rule, index) => {
    ...
    for (const [value, label] of [[ACTION_KEEP, "保留"], [ACTION_DROP, "丢弃"]] as const) {
    ...
    tr.appendChild(pairCell(
      textInput(rule.minLength ?? "", 3, "最小"),
      textInput(rule.maxLength ?? "", 3, "最大"),
      (v) => { rule.minLength = v === "" ? null : Number(v); },
      (v) => { rule.maxLength = v === "" ? null : Number(v); }
    ));
...
function listCell(input: HTMLInputElement, onInput: (value: string) => void): HTMLTableCellElement {
...
function draftRuleSet(): BarcodeRuleSet {
  return {
    defaultAction: $<HTMLSelectElement>("ruleDefaultAction").value || ACTION_KEEP,
    ignoreCase: $<HTMLInputElement>("ruleIgnoreCase").checked,
    rules: editing
  };
}
...
async function testRules(): Promise<void> {
  const text = $<HTMLTextAreaElement>("ruleTestCodes").value;
  const codes = text
    .split(/\r?\n|,|;/)
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
  ...
  const body = $<HTMLTableSectionElement>("ruleTestRows");
  clear(body);
...
async function loadFiltered(): Promise<void> {
  ...
  const body = $<HTMLTableSectionElement>("filteredRows");
  clear(body);
