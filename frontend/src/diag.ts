...
import type { DiagFileItem, DiagOverviewResponse, DiagSourceInfo } from "./types.js";

/** 勾选进打包的来源（默认全选） */
const checked = new Set<string>();
let overview: DiagOverviewResponse | null = null;
...
function setRange(daysBack: number): void {
  ...
  $<HTMLInputElement>("diagFrom").value = dateText(from);
  $<HTMLInputElement>("diagTo").value = dateText(to);
}
...
export async function refreshDiag(): Promise<void> {
  ...
  if (checked.size === 0) {
    for (const source of overview.sources) checked.add(source.id);
  }
...
function renderSources(list: DiagSourceInfo[]): void {
  ...
  for (const source of list) {
    ...
    if (source.id === current) card.classList.add("picked");
    ...
    pick.addEventListener("click", (event) => {
      ...
      if (pick.checked) checked.add(source.id);
      else checked.delete(source.id);
...
function numBox(value: string, label: string): HTMLElement {
...
async function loadFiles(): Promise<void> {
  if (!current) return;
  const from = $<HTMLInputElement>("diagFrom").value || dateText(new Date());
  const to = $<HTMLInputElement>("diagTo").value || dateText(new Date());

  const res = await api.diagFiles(current, dateKey(from), dateKey(to));
  const rows = $<HTMLTableSectionElement>("diagFileRows");
  clear(rows);
  if (res.status !== 200 || !res.data) {
...
function fileRow(sourceId: string, file: DiagFileItem): HTMLTableRowElement {
  const tr = el("tr");
...
async function openTail(sourceId: string, fileName: string): Promise<void> {
