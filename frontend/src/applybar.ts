...
/** 应用基线：服务器上当前的触发模式与相机清单，用来判断"改了没有" */
interface Baseline {
  triggerMode: string;
  cameras: string[];
}

let baseline: Baseline = { triggerMode: "", cameras: [] };
let readCameras: () => string[] = () => [];
...
export function triggerModeToUi(mode: string | null | undefined): string {
...
export function triggerModeLabel(value: string): string {
  if (value === "hard") return "硬触发（光电）";
  if (value === "soft") return "软触发（调试用）";
  if (value === "free") return "自由拉流（狂扫）";
  return value || "未设置";
}

function normalizeCameras(list: string[]): string[] {
  return list
...
function sameCameras(a: string[], b: string[]): boolean {
...
export function setApplyBaseline(triggerMode: string, cameras: string[]): void {
  baseline = { triggerMode: triggerMode ?? "", cameras: cameras.slice() };
  refreshApplyBar();
}

/** 界面任何一处改动后调用：重新算"有没有未保存的改动" */
export function refreshApplyBar(): void {
  if (busy) return;
...
/** 应用开始：禁用按钮 + 计时 */
export function applyBarStart(): void {
  busy = true;
  startedAt = Date.now();
  const button = $<HTMLButtonElement>("btnApply");
...
/** 应用结束 */
export function applyBarFinish(ok: boolean, conclusion: string): void {
  busy = false;
  if (ticker !== null) {
...
/** 应用失败/异常（没拿到结构化结果时） */
export function applyBarError(message: string): void {
  applyBarFinish(false, message);
}

/** 组装按钮：传两个"读当前界面值"的回调，改动时条上能自己判断 */
export function initApplyBar(getCameras: () => string[], getTrigger: () => string): void {
