...
import { $, badge, cell, clear, dash, el } from "./dom.js";
import type {
  MonitorAlert,
  MonitorCameraStatus,
  MonitorEvent,
...
export function initMonitor(): void {
  eventRowsEl = $<HTMLTableSectionElement>("monEventRows");
  camRowsEl = $<HTMLTableSectionElement>("monRows");
  alertBarEl = $("alertBar");
  monMsgEl = $("monMsg");
...
export function renderMonitorSnapshot(data: MonitorSnapshot): void {
  snapshot = data;
...
export function renderAlert(alert: MonitorAlert): void {
  if (!alert) return;
  const index = alerts.findIndex((a) => a.id === alert.id);
  if (alert.active) {
    if (index >= 0) alerts[index] = alert;
    else alerts = [alert, ...alerts];
  } else if (index >= 0) {
    alerts[index] = alert;
  } else {
    alerts = [alert, ...alerts];
...
function renderSummary(s: MonitorSummary | undefined): void {
  if (!s) return;
...
function renderCameras(list: MonitorCameraStatus[]): void {
  clear(camRowsEl);
  const sorted = [...list].sort((a, b) => a.camera.localeCompare(b.camera));

  for (const item of sorted) {
    const tr = el("tr");
    if (!item.online) tr.className = "offline";
    ...
    if ((item.alerts ?? []).length === 0) {
      ...
      for (const a of item.alerts) {
        alertTd.appendChild(
          el("span", CODE_LABEL[a.code] ?? a.code, "badge " + (a.severity === "critical" ? "err" : "warn"))
        );
...
function renderAlertBar(s: MonitorSummary | undefined, list: MonitorAlert[]): void {
  ...
  if (active.length === 0) {
    alertBarEl.appendChild(
      el("span", "相机状态正常：" + (s?.online ?? 0) + " / " + (s?.cameras ?? 0) + " 台在线，平均在线率 " + fmtRate(s?.averageOnlineRatePercent), "")
    );
    return;
  }
...
function isCritical(a: MonitorAlert): boolean {
  return a.severity === "critical";
}

function renderEvents(): void {
  clear(eventRowsEl);
  const onlyActive = $<HTMLInputElement>("monOnlyActive").checked;
  const filtered = onlyActive ? events.filter((e) => e.event === "alert-raised") : events;

  for (const item of filtered) {
    const tr = el("tr");
    if (item.event === "offline" || item.event === "alert-raised") tr.className = "offline";
    ...
    tr.appendChild(cell(
      item.offlineDurationMs > 0 ? (item.offlineDurationMs / 1000).toFixed(1) + " 秒" : "—",
      "muted"
    ));
...
export async function refreshMonitor(showMessage = false): Promise<void> {
  const [summary, cameras] = await Promise.all([api.monitorSummary(), api.monitorCameras()]);
  if (summary.data) renderSummary(summary.data);
  if (cameras.data) renderCameras(cameras.data);
...
export function initMonitorConfig(): void {
...
export async function refreshMonitorConfig(): Promise<void> {
...
function fillMonitorConfig(options: MonitorOptions): void {
  const o = options ?? DEFAULT_OPTIONS;
  $<HTMLInputElement>("monEnabled").checked = o.enabled !== false;
  $<HTMLInputElement>("monHeartbeat").value = String(o.heartbeatTimeoutSeconds ?? 60);
  $<HTMLInputElement>("monOfflineAlert").value = String(o.offlineAlertSeconds ?? 10);
  $<HTMLInputElement>("monFreqCount").value = String(o.frequentOfflineCount ?? 3);
  $<HTMLInputElement>("monFreqWindow").value = String(o.frequentOfflineWindowMinutes ?? 30);
  $<HTMLInputElement>("monRatePercent").value = String(o.onlineRateAlertPercent ?? 95);
  $<HTMLInputElement>("monRateWindow").value = String(o.onlineRateWindowMinutes ?? 60);
  $<HTMLInputElement>("monCheckInterval").value = String(o.checkIntervalSeconds ?? 5);
  $<HTMLInputElement>("monRetention").value = String(o.eventRetentionDays ?? 30);
}

async function saveMonitorConfig(): Promise<void> {
  const options: MonitorOptions = {
    enabled: $<HTMLInputElement>("monEnabled").checked,
