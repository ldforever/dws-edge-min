...
import { $, badge, csvCell, cell, clear, downloadText, el, notify } from "./dom.js";
import type { ShiftDef, StatsBoardResponse, StatsGroupRow } from "./types.js";
...
let board: StatsBoardResponse | null = null;
...
export function initStats(): void {
  ...
  $<HTMLSelectElement>("statsDim").addEventListener("change", () => void refreshStats());
...
function dateText(offsetDays: number): string {
  const day = new Date();
...
function setRange(daysBack: number): void {
  $<HTMLInputElement>("statsFrom").value = dateText(daysBack);
  $<HTMLInputElement>("statsTo").value = dateText(0);
}

export async function refreshStats(): Promise<void> {
  const from = $<HTMLInputElement>("statsFrom").value || dateText(0);
  const to = $<HTMLInputElement>("statsTo").value || dateText(0);
  const dimension = $<HTMLSelectElement>("statsDim").value;
  const deviceId = $<HTMLInputElement>("statsDevice").value.trim();
...
function renderBoard(data: StatsBoardResponse): void {
  ...
  const rows = $<HTMLTableSectionElement>("statsRows");
  clear(rows);
...
function groupRow(group: StatsGroupRow, max: number): HTMLTableRowElement {
  const tr = el("tr");
  if (group.noreadRatePercent >= 50) tr.className = "offline";
...
function exportCsv(): void {
  ...
  const all: StatsGroupRow[] = [board.totals, ...board.groups];
  ...
  downloadText("dws-stats-" + board.dimension + "-" + board.from.replace(/-/g, "") + "-" + board.to.replace(/-/g, "") + ".csv",
    lines.join("\r\n"), "text/csv");
}
...
async function refreshShifts(): Promise<void> {
  ...
  if (!res.data) return;
  const rows = $<HTMLTableSectionElement>("shiftRows");
  clear(rows);
...
function shiftRow(shift: ShiftDef, span?: string): HTMLTableRowElement {
  const tr = el("tr");

  const nameTd = el("td");
  const name = el("input", undefined, "rule-input") as HTMLInputElement;
  name.type = "text";
  name.size = 12;
  ...
  const startTd = el("td");
  const start = el("input", undefined, "rule-input") as HTMLInputElement;
  start.type = "text";
  start.size = 6;
  ...
  const endTd = el("td");
  const end = el("input", undefined, "rule-input") as HTMLInputElement;
  end.type = "text";
  end.size = 6;
  end.value = shift.end;
...
async function saveShifts(): Promise<void> {
  const rows = Array.from($("shiftRows").children) as HTMLTableRowElement[];
  const shifts: ShiftDef[] = [];
  for (const row of rows) {
    const inputs = row.querySelectorAll("input");
    if (inputs.length < 3) continue;
    const name = (inputs[0] as HTMLInputElement).value.trim();
    const start = (inputs[1] as HTMLInputElement).value.trim();
    const end = (inputs[2] as HTMLInputElement).value.trim();
