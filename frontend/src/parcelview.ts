...
import { $, clear, el, positionLabel } from "./dom.js";
...
function keyOf(p: ParcelRecord): string {
  return p.traceId ?? (p.time ?? "") + "|" + (p.codes ?? []).join(",");
}

export function initParcelView(): void {
  listEl = $("pvList");
  imageEl = $<HTMLImageElement>("pvImage");
  boxLayerEl = $("pvBoxLayer");
  emptyEl = $("pvEmpty");
  infoEl = $("pvInfo");
  followBtn = $<HTMLButtonElement>("pvFollow");

  followBtn.addEventListener("click", () => {
    following = true;
    updateFollowButton();
    listEl.scrollTop = 0;
    const newest = listEl.firstElementChild as HTMLElement | null;
    if (newest?.dataset.key) select(newest.dataset.key);
  });
...
/** 新包裹 / 同一条的更新：都走这里 */
export function upsertParcel(p: ParcelRecord, flash: boolean): void {
  const key = keyOf(p);
  records.set(key, p);
  ...
  if (flash) row.className = "plistrow new";
...
export function loadParcels(list: ParcelRecord[]): void {
  for (let i = list.length - 1; i >= 0; i--) {
    const p = list[i];
    if (p) upsertParcel(p, false);
  }
  following = true;
  updateFollowButton();
}
...
function buildRow(p: ParcelRecord, key: string): HTMLElement {
...
function codeText(p: ParcelRecord): string {
  const codes = p.codes ?? [];
  return codes.length > 0 ? codes.join(" ") : "NOREAD";
}

function positionText(p: ParcelRecord): string {
  const first = (p.codeDetails ?? [])[0];
  return first?.position ? positionLabel(first.position) : "—";
}

function weightText(p: ParcelRecord): string {
  const grams = p.weightGrams ?? 0;
  return grams > 0 ? (grams / 1000).toFixed(2) + " kg" : "—";
}

function stateText(p: ParcelRecord): string {
  if ((p.codeCount ?? 0) === 0) return "无码";
  if (p.complete === false) return "待补全";
  if ((p.updates ?? 0) > 1) return "已读码 ×" + p.updates;
  return "已读码";
}

/** 画大图 + 绿框 + 单号 */
function renderViewer(p: ParcelRecord | null): void {
  ...
  if (imageEl.getAttribute("src") !== url) imageEl.src = url;

  drawBoxes(p);
}

function volumeText(p: ParcelRecord): string {
 ...
 */
function drawBoxes(p: ParcelRecord): void {
  ...
  for (const box of boxes) {
    ...
    if (box.code) {
      ...
      for (const pt of points) {
        if (typeof pt[0] === "number" && pt[0] < minX) minX = pt[0];
        if (typeof pt[1] === "number" && pt[1] < minY) minY = pt[1];
      }
