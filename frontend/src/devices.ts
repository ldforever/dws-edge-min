...
import { $, cell, clear, csvCell, downloadText, el, positionLabel, FACE_ORDER, notify } from "./dom.js";
import { statusText } from "./realtime.js";
import type { CameraRecord, DeviceView } from "./types.js";
...
export function initDevices(): void {
  rowsEl = $<HTMLTableSectionElement>("devRows");
...
export function scheduleDevicesRefresh(): void {
  if (!isActive()) return;
...
function render(): void {
  ...
  const msg = $("posMsg");
  if (msg.textContent === "保存中…" && dirty.size === 0) msg.textContent = "";
...
function renderFaces(v: DeviceView): void {
  ...
  for (const name of FACE_ORDER) {
    ...
    card.appendChild(
      el("div", face.total ? "在线 " + face.online + (face.offline ? " / 离线 " + face.offline : "") : "未使用", "who")
    );
  ...
  for (const row of map) {
    for (const name of row) {
      ...
      if (name) {
        ...
        if (face?.total) box.className = "has";
      }
...
function renderTable(v: DeviceView): void {
  clear(rowsEl);
...
function positionCell(c: CameraRecord, options: string[]): HTMLTableCellElement {
  const td = el("td");
...
async function savePositions(): Promise<void> {
  ...
  for (const [key, value] of dirty) positions[key] = value;
