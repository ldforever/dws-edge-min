...
function build(name: string, size: number): SVGSVGElement | null {
  const shape = SHAPES[name];
  if (!shape) return null;

  const markup =
    '<svg xmlns="' + SVG_NS + '" viewBox="0 0 24 24" width="' + size + '" height="' + size +
...
export function initIcons(root: ParentNode = document): void {
  for (const host of Array.from(root.querySelectorAll<HTMLElement>("[data-icon]"))) {
