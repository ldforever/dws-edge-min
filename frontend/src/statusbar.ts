...
function renderCameras(
  ...
  if (host.state !== "running") {
    if (host.ageSeconds > HOST_STATUS_STALE_SECONDS) {
      set("sbCameras", "unknown", "相机：状态过期",
        "宿主状态文件已经 " + Math.round(host.ageSeconds) + " 秒没更新（超过 " + HOST_STATUS_STALE_SECONDS + " 秒即视为过期）。\n" +
...
function sdkReason(code: number): { short: string; long: string } | null {
  if (code === 3000) {
