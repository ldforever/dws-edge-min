...
import type { DeviceView, DownstreamResponse, HostStatus, MonitorCameraStatus, Stats } from "./types.js";
...
export type VerdictKind = "ok" | "warn" | "bad" | "unknown";

export interface HostVerdict {
  kind: VerdictKind;
  /** 一句话结论，直接显示（"SDK 发现不到" / "宿主没在跑"…） */
  label: string;
  /** 展开的原因（悬停提示用），可能多行 */
  note: string;
  /** 相机数据可不可信：只有这里为 true 时，平台那份"在线"才作数 */
  camerasReliable: boolean;
...
const verdictListeners = new Set<(value: HostVerdict) => void>();

/** 发布新结论：状态条每 5 秒刷一次就喂一次，订阅方（实时页）跟着重画 */
export function setHostVerdict(next: HostVerdict): void {
  verdict = next;
...
export function getHostVerdict(): HostVerdict {
  return verdict;
}

export function onHostVerdict(listener: (value: HostVerdict) => void): void {
  verdictListeners.add(listener);
}

/** SDK 返回码 → 人话（3000 没相机 / 3001 被占用 / 2200 没加密狗） */
export function sdkReason(code: number): string | null {
  if (code === 3000) {
...
export function verdictFromHost(host: HostStatus | null): HostVerdict {
  if (!host) {
    return {
      kind: "unknown",
      label: "状态未知",
...
/** 五个胶囊的 id（顺序 = 页头从左到右） */
const CAPSULES: { id: string; name: string }[] = [
 ...
 */
function label(el: HTMLElement): HTMLElement {
  const found = el.querySelector<HTMLElement>(".sbar-text");
...
function set(id: string, state: CapsuleState, text: string, title?: string): void {
  const el = $(id);
...
function renderHost(res: ApiResult<HostStatus>): string {
  if (!res.data) {
...
function renderChannel(res: ApiResult<{ available: boolean; pipeName: string; message: string }>, hostState: string): void {
  if (!res.data) {
...
function renderCameras(
  res: ApiResult<DeviceView>,
  host: HostStatus | null,
  cameras: MonitorCameraStatus[] | null
): void {
  ...
  if (!current.camerasReliable) {
    set("sbCameras", current.kind === "ok" ? "unknown" : current.kind,
      "相机：" + current.label, current.note + "\n" + detail);
    return;
  }
...
function deviceDetail(view: DeviceView, cameras: MonitorCameraStatus[] | null): string {
  const lines: string[] = [];
...
function freshestHeartbeatAge(cameras: MonitorCameraStatus[] | null): number | null {
...
function renderDownstream(res: ApiResult<DownstreamResponse>): void {
  ...
  if (stats.httpMode) {
    ...
    set("sbDownstream", trouble ? "warn" : "ok", "下游：HTTP 推送" + queued,
      "目标：" + (stats.httpTarget ?? "—") + "\n" + counters);
    return;
  }
  ...
  if (stats.serverMode) {
    ...
    set("sbDownstream", connected ? "ok" : "warn", "下游：监听中（" + stats.clientCount + " 连接）" + queued,
      "监听：" + (stats.listenTarget ?? "—") + "\n" + counters);
    return;
  }

  const title = "目标：" + (stats.target || "—") + "\n" + counters
    + (stats.lastError ? "\n最近错误：" + stats.lastError : "");
  set("sbDownstream", stats.connected ? "ok" : "warn",
    "下游：" + (stats.connected ? "已连接" : "未连接") + queued, title);
}

function renderDisk(res: ApiResult<Stats>): void {
