...
function setRange(daysBack: number): void {
  ...
  $<HTMLInputElement>("histFrom").value = isoDate(from);
  $<HTMLInputElement>("histTo").value = isoDate(today);
}
...
function readFilter(): HistoryFilter {
  const limit = Number($<HTMLSelectElement>("histLimit").value) || 50;
  return {
    from: $<HTMLInputElement>("histFrom").value,
    to: $<HTMLInputElement>("histTo").value,
    code: $<HTMLInputElement>("histCode").value.trim(),
    deviceId: $<HTMLInputElement>("histDevice").value.trim(),
    noread: $<HTMLSelectElement>("histNoread").value as HistoryFilter["noread"],
    dispatchState: $<HTMLSelectElement>("histDispatch").value as HistoryFilter["dispatchState"],
    hasImage: $<HTMLSelectElement>("histImage").value as HistoryFilter["hasImage"],
    limit,
    offset: currentOffset
  };
}

async function page(direction: number): Promise<void> {
  const filter = readFilter();
  const next = currentOffset + direction * filter.limit;
  if (next < 0) return;
  if (lastResult && next >= lastResult.total) return;
  currentOffset = next;
...
function render(result: HistoryResult, filter: HistoryFilter): void {
  const body = $<HTMLTableSectionElement>("histRows");
  clear(body);
...
function renderRow(record: ParcelRecord): HTMLTableRowElement {
  const tr = el("tr");
  ...
  const codeTd = el("td");
  const codes: CodeDetail[] = record.codeDetails?.length
    ? record.codeDetails
  ...
  if (codes.length) {
    ...
    codes.forEach((c, i) => {
      ...
      const tags: string[] = [];
      if (c.position) tags.push(positionLabel(c.position));
      if (c.kind && c.kind !== "unknown") tags.push(c.kind.toUpperCase());
      if (tags.length) codeTd.appendChild(el("span", " (" + tags.join("/") + ")", "tag"));
    });
  } else {
